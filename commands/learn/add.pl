...
use LearnDB qw/$RTERM_INDEXED $RTERM $RTEXT report_error/;
use LearnDB::Cmd;
use Helper;
use utf8;
...
eval {
  if ($ARGV[1] =~ /^$RTERM_INDEXED $RTEXT/) {
    print LearnDB::Cmd::insert_entry(LearnDB::unquote($1), $2, $3);
  } elsif ($ARGV[1] =~ /^$RTERM $RTEXT/) {
    print LearnDB::Cmd::insert_entry(LearnDB::unquote($1), -1, $2);
  } else {
