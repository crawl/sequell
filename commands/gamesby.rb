...
full_query = \
q.select("MAX(#{LOG2SQL['name']}), COUNT(*), MIN(#{LOG2SQL['starttime']}), " +
         "MAX(#{LOG2SQL['endtime']}), " +
         "MAX(points), SUM(points), SUM(turns), SUM(realtime)", false)
...
if rows.empty? || rows[0][1].to_i == 0
  ...
  puts "#{q.argstr} has played #{ngames} game#{plural}, between " +
      "#{datestr(tstart)} and #{datestr(tend)}, ascended #{winstr(win_count, ngames)}, " +
